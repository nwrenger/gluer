// Generated by gluer v0.9.0 — https://github.com/nwrenger/gluer
...
namespace api {
    ...
    */
	export async function add_root(
		path: number,
		data: Result<
			Hello<Hello<Huh<Huh<Hello<Age, string>>>, string>, string>
		>,
	...
	export async function fetch_root(
		...
		return fetch_api(
			`${PREFIX}/${encodeURIComponent(path)}?${new URLSearchParams(queryMap).toString()}`,
			{
				method: "GET",
			},
		);
	...
	export async function get_alphabet(
		pathTuple: [Alphabet, S],
	): Promise<[Alphabet, S]> {
		return fetch_api(
			`${PREFIX}/char/${encodeURIComponent(pathTuple[0])}/metadata/${encodeURIComponent(pathTuple[1])}`,
			{
				method: "GET",
			},
		);
